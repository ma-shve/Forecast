DECLARE @StartDateNew DATETIME = DATETIME2FROMPARTS(2018,11,01,00,00,00,00,00)/*(2018,08,01)*/
...
FROM 
	...
	GROUP BY 
		...
		T2._Period
	HAVING 
		(CAST(SUM(T2._Fld16878) AS NUMERIC(32, 8))) <> 0.0) T1
...
WHERE 	
	T3._Fld3394 = '1016'	
	/*AND (T4._Description = 'Центральный')*/
GROUP BY 	
